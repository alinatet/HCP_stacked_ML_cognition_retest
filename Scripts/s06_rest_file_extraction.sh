...
path=/media/DataD800/Alina/retest_set/3T_rfMRI_REST_fix
...
mkdir $path/folders/
mkdir $path/z_analysis/

for type in ${types[@]}
do
    mkdir $path/z_analysis/$type
done
...
unzip -q -o $path/archives/$subject*.zip -d $path/folders/
    ...
    for type in ${types[@]}
    do
        cp $path/folders/$subject/MNINonLinear/Results/$type/*MSMAll_hp2000_clean* $path/z_analysis/$type/$subject.$type.Atlas_MSMAll_hp2000_clean.dtseries.nii
    done
